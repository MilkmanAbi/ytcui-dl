...
public:
    ...
    // --- AUDIO: STREAM (stream_url without &range= for mpv HTTP playback) ---
    static std::string select_best_audio_stream(const std::vector<StreamFormat>& fmts,
                                                 const std::string& prefer_codec = "") {
        auto* f = pick_audio(fmts, prefer_codec, 0, false);
        if (!f) return "";
    ...
    static std::string resolve_format_string(const std::string& sel,
        ...
        if (base == "bestaudio" || base == "ba" || base == "audio") {
            ...
            if (matching.empty()) {
                // Fallback: any audio
                auto* f = pick_audio(fmts, "", 0, !for_streaming);
                if (!f) return "";
            ...
            return for_streaming ? stream_or_url(*matching[0]) : matching[0]->url;
        }
...
private:
    ...
    // Return stream_url if available, else url
