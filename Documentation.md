...
```cpp
// Get audio URL (stream_url, NOT download url)
auto info = ytfast::yt_get_formats(video_id);
std::string audio = ytfast::InnertubeClient::select_best_audio_stream(
    info.formats, "aac");  // prefer aac for mpv compat

// Build mpv command
std::string cmd = "mpv --no-video"
    " --user-agent='" + std::string(ytfast::ANDROID_UA) + "'"
    " '" + audio + "'";
system(cmd.c_str());
```

**Important**: Pass the Android User-Agent to mpv. YouTube CDN validates the UA matches what was used in the player API request. Without it you may get 403 on some videos.
...
### mpv says "HTTP error 403"

Pass the Android User-Agent to mpv:
```bash
mpv --user-agent='com.google.android.youtube/21.02.35 (Linux; U; Android 11) gzip' "$URL"
```
